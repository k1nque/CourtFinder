...
async def contract_dispute_yes(msg: types.Message, state: FSMContext):
...
async def contract_dispute_no(msg: types.Message, state: FSMContext):
...
async def transportation_yes(msg: types.Message, state: FSMContext):
    await msg.answer("Укажите адрес перевозчика")
    # TODO Вывод финального ответа


@router.message(StateFilter(States.Transportation), F.text == "Нет")
async def transportation_no(msg: types.Message, state: FSMContext):
...
async def damage_compensation(msg: types.Message, state: FSMContext):
    await msg.answer("Укажите ваше (любое):\nМесто жительства\nМесто причинения вреда\nАдрес ответчика")
    # TODO Вывод финального ответа


@router.message(StateFilter(States.DamageCompensation), F.text == "Нет")
async def damage_compensation_no(msg: types.Message, state: FSMContext):
...
async def contract_determined_jurisdiction_yes(msg: types.Message, state: FSMContext):
...
async def contract_determined_jurisdiction_no(msg: types.Message, state: FSMContext):
...
async def address_specified_yes(msg: types.Message, state: FSMContext):
...
async def labor_rights_protection(msg: types.Message, state: FSMContext):
...
async def ships_collision(msg: types.Message, state: FSMContext):

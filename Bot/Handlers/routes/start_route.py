...
@StartRouter.message(StateFilter(StartRouteStates.IsItCriminalCase), Text("Да"))
async def is_it_criminal_case_yes(msg: types.Message, states: FSMContext):
    await msg.answer("Рекомендуем Вам обратиться в иной правоохранительный орган (иди в полицию, дурашка)")
    await state.set_state(None)


@StartRouter.message(StateFilter(StartRouteStates.IsItCriminalCase), Text("Нет"))
async def is_it_criminal_case_no(msg: types.Message, states: FSMContext):
    await msg.answer("Слышали ли Вы о третейских судах?")
    await state.set_state(StartRouteStates.IsItTreteyCourt)


@StartRouter.message(StateFilter(StartRouteStates.IsItTreteyCourt), Text("Нет"))
async def is_it_tretey_court_no(msg: types.Message, states: FSMContext):
    await msg.answer("дело рассмкатривалось в суде ранее?")
    await state.set_state(StartRouteStates.WasCaseBefore)


@StartRouter.message(StateFilter(StartRouteStates.WasCaseBefore), Text("Да"))
async def was_case_before_yes(msg: types.Message, states: FSMContext):
    await msg.answer("Это был российский суд?")
    await state.set_state(StartRouteStates.IsItRussianCourt)


@StartRouter.message(StateFilter(StartRouteStates.WasCaseBefore), Text("Нет"))
async def was_case_before_no(msg: types.Message, states: FSMContext):
